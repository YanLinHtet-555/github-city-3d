...
const B_COLOR = [0x02040a, 0x03050c, 0x03050e, 0x02040e, 0x01030c];
const W_COLOR = [0x080e1e, 0x1a44aa, 0x2266dd, 0x11aaff, 0x66ddff];
const E_INT   = [0, 1.1, 2.0, 3.2, 5.0];
...
function winTex(lv) {
  ...
  const WIN_COLS = [
    [28,72,200],[40,100,230],[20,130,255],[50,170,255],[110,200,255],[180,220,255],
  ];
...
function makeBuilding(count, lv, seed) {
  ...
  if (lv >= 1) {
    ...
    lobby.position.y = lh / 2; group.add(lobby);
  }

  if (lv >= 3 && h > 7) {
    const sb1H = h * 0.26, sb1W = fw * 0.78;
    const sb1T = winTex(lv).clone();
    sb1T.repeat.set(sb1W/0.88, sb1H/0.88); sb1T.offset.set(rng(0,1), rng(0,1));
    ...
    sb1.position.y = h + sb1H / 2; group.add(sb1);

    if (lv === 4 && h > 16) {
      const sb2H = h * 0.12, sb2W = fw * 0.55;
      const sb2T = winTex(lv).clone();
      sb2T.repeat.set(sb2W/0.88, sb2H/0.88); sb2T.offset.set(rng(0,1), rng(0,1));
      ...
      sb2.position.y = h + sb1H + sb2H / 2; group.add(sb2);
    }
  }
  ...
  if (lv >= 3 && h > 5) {
    ...
    ant.position.y = h + aH / 2; group.add(ant);
    ...
    _blinkMeshes.push(bl); group.add(bl);
...
function makeDrone(x, y, z, seed) {
  const g = new THREE.Group();
  g.add(new THREE.Mesh(new THREE.SphereGeometry(0.09,6,4), new THREE.MeshBasicMaterial({ color: 0xffffff })));
...
function buildCity(contributions, username) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const d = makeDrone((Math.random()-0.5)*W*0.7, h, (Math.random()-0.5)*20, Math.random());
...
// ── API — direct call (no proxy needed on GitHub Pages) ───────────────────────
